...
//    private static final String TOMCAT_BASE_DIR = "/usr/local/apache-tomcat-6.0.29-dev/webapps/PIPE2/";
    ...
    */
    public String SubmitGOEnrichment(Namelist namelist, String enrichmentType) throws Exception {
        ...
        if (!goEnrichmentServiceAvailable())
            throw new Exception("GO Enrichment Service temporarily unavailable.  If proplem persists, please contact administrators.");
    ...
    public String[][] checkGOJobStatus(String refid, String resultsFilename) throws Exception {
        ...
        if (completeIndicatorFile.exists()) {
            //COPY FILE/FOLDER OF RESULTS BACK TO WEB DIRECTORY
    ...
    public static void copy_file(String fromFileName, String toFileName)
            throws IOException {
    ...
    private void submitJobToCluster(File directory) throws IOException {
        try {
            String qsub_str;
            String cmd = "ssh -x -i ~/.ssh/id_dsa corra@moog.systemsbiology.net qsub -N PIPE2 -l urgent " + directory.toString() + File.separator + "submitJobToCluster.csh";
            System.out.println("Execute:" + cmd);
            Process qsub_proc = Runtime.getRuntime().exec(cmd);
     ...
     *
     * @param directory
     * @param finalLocationDirectoit looks like PIPE2 request of node which running as corra user looks like allocated to qconvert queuery
    ...
    private void writeClusterSubmitionScript(File pbsDirectory, String pbsScriptFilename) throws IOException {
        ...
        writer.write("#$ -M mbrusniak@systemsbiology.org\n#$ -m abe\n");
        writer.write("#$ -o " + PIPE2_PBS_GO_DIR + "SGEoutput/PIPE2.o\n");
        writer.write("#$ -e " + PIPE2_PBS_GO_DIR + "SGEoutput/PIPE2.e\n");
        writer.write("#$ -l h_rt=200:00:00\n");
        writer.write("#$ -l h_vmem=4294967296\n");
        writer.write("#$ -p corra\n");
    ...
    private String makeNewDirectory(String enrichmentType, String organism) {
        ...
        if (organism.equalsIgnoreCase("Homo sapiens") || organism.equalsIgnoreCase("human"))
            fixedOrganism = "human";
        else if (organism.equalsIgnoreCase("mus musculus") || organism.equalsIgnoreCase("mouse"))
            fixedOrganism = "mouse";
        else if (organism.equalsIgnoreCase("Saccharomyces cerevisiae") || organism.equalsIgnoreCase("yeast"))
            fixedOrganism = "yeast";
        else if (organism.equalsIgnoreCase("Rattus norvegicus") || organism.equalsIgnoreCase("rat"))
            fixedOrganism = "rat";
    ...
    private void writeParamFile(Namelist namelist, String enrichmentType, File directory, String outFilename, String filename) throws Exception {
        ...
        for (int g = 0; g < namelist.getNames().length; g++)
            writer.write(namelist.getNames()[g] + "\n");
        writer.close();
    }
    ...
    private int estimateTimeToCompletion(int numGenes, String organism, String ontologyType) {
        //todo: remove the true directly below this line
        /*return 6; */
        if (ontologyType == null || organism == null || numGenes == 0)
            return 1;
        if (organism.equals("Homo sapiens")) {
            ...
            if (numGenes < 76) {
                ...
                || organism.equals("Saccharomyces cerevisiae")) {
            if (numGenes < 76) {
                if (ontologyType.equals("BP")) {
                    return 130;
                } else if (ontologyType.equals("MF")) {
