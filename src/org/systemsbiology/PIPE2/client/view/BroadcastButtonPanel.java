...
public class BroadcastButtonPanel extends Grid implements SourcesChangeEvents {
	...
	public String getDataSource(){
		if(dataSourceLB.getItemCount() > 0)
			return dataSourceLB.getItemText(dataSourceLB.getSelectedIndex());
		else
			return "";
	}
    
	public String getTarget(){
		if(targetsLB.getItemCount() > 0)
			return targetsLB.getItemText(targetsLB.getSelectedIndex());
		else
			return "";
	}
	...
	public void setSources(ArrayList<String> newSources){
		...
		for(int i = 0; i < newSources.size(); i++){
			...
			dataSourceLB.addItem(newSource);
			if(newSource.equals(currentSelection))
				currentSelectionsNewIdx = i;
		}
	...
	public void setTargets(ArrayList<String> newTargets){
		...
		for(int i = 0; i < newTargets.size(); i++){
			...
			if(newTarget.equals(currentSelection))
				currentSelectionsNewIdx = i;
		}
    ...
    public void setSelectedDataSource(String selectSource){
        for(int i = 0; i < dataSourceLB.getItemCount(); i++)
            if(dataSourceLB.getItemText(i).equals(selectSource))
                dataSourceLB.setItemSelected(i, true);
    }
}
